...
-- ============================================================
-- STEP 3: CREATE FORCE DELETE FUNCTION
-- ============================================================

CREATE OR REPLACE FUNCTION force_delete_project_type(
    p_project_type_name TEXT
)
RETURNS JSON AS $$
DECLARE
...
BEGIN
    ...
    result := json_build_object(
        ...
        'message', 'Project type and ' || activity_count || ' activities force deleted',
        'project_type', p_project_type_name,
...
COMMENT ON FUNCTION force_delete_project_type(TEXT) IS 
'Force deletes a project type and ALL its activities. Use with caution.';
...
SELECT '✅ Created force_delete_project_type function' as status;

-- ============================================================
-- STEP 6: VERIFY FOREIGN KEY CONSTRAINT
-- ============================================================

-- Simple check for foreign key constraint
SELECT 
    'Foreign key constraint check' as status,
    CASE 
        WHEN EXISTS (
        ...
        ELSE '❌ Foreign key constraint not found'
...
SELECT '🎉 Safe delete functions created successfully!' as status,
       ...
       'Use force_delete_project_type() for force deletion' as note3;
