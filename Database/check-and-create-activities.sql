...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'activities') THEN
        ...
        RAISE NOTICE 'Activities table created successfully with 41 activities!';
        
    ELSE
        RAISE NOTICE 'Activities table already exists. Skipping creation.';
    END IF;
END $$;

-- عرض النتيجة
SELECT 
    'Activities table is ready!' as status,
