-- ============================================================
-- حل المشاكل الأمنية الإضافية - Database Linter
-- ============================================================
...
-- ✅ إصلاح View: user_permission_activity
DROP VIEW IF EXISTS public.user_permission_activity;
CREATE VIEW public.user_permission_activity AS
SELECT 
    u.email,
    u.role,
    COUNT(pal.id) as permission_changes
FROM public.users u
LEFT JOIN public.permissions_audit_log pal ON u.id = pal.user_id
...
SELECT '✅ تم حل جميع المشاكل الأمنية الإضافية بنجاح!' as status;
