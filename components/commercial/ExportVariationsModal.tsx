...
interface ExportVariationsModalProps {
  isOpen: boolean
  onClose: () => void
...
const allColumns: ColumnOption[] = [
  { key: 'auto_generated_unique_reference_number', label: 'Reference Number', selected: true },
...
export function ExportVariationsModal({ 
  isOpen, 
  onClose, 
  variations,
  getBOQItemDescription 
}: ExportVariationsModalProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportDataArray = variations.map(variation => {
        const row: any = {}
        selectedColumns.forEach(columnKey => {
          let value = variation[columnKey as keyof ContractVariation]
          
          // Special handling for item_description - show description if function provided
          if (columnKey === 'item_description' && getBOQItemDescription && typeof value === 'string') {
      ...
      const filename = generateFilename('Contract_Variations')
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <CardHeader className="flex-shrink-0">
          <div className="flex items-center justify-between">
            <CardTitle>Export Contract Variations</CardTitle>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
              <p className="text-sm text-blue-700 dark:text-blue-300">
                <strong>Exporting:</strong> {variations.length} filtered variation(s)
              </p>
        ...
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 p-4 flex justify-end gap-2">
          ...
          <Button
            ...
            disabled={exporting || selectedColumns.size === 0 || variations.length === 0}
          >
