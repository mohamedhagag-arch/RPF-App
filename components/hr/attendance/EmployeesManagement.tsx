...
import { 
  ...
  FileText, FileSpreadsheet, CheckSquare, Square, RefreshCw, Briefcase, CreditCard
} from 'lucide-react'
...
export function EmployeesManagement() {
  ...
  const [viewingIDCard, setViewingIDCard] = useState<AttendanceEmployee | null>(null)
  ...
  const fetchEmployees = async () => {
    try {
      ...
      // Ensure all employees have QR codes in unified format
      // This runs in the background to avoid blocking the UI
      Promise.all(
        employeesList.map(async (emp: AttendanceEmployee) => {
          try {
            await ensureQRCode(emp)
          } catch (err) {
            console.error(`Failed to ensure QR code for ${emp.employee_code}:`, err)
          }
        })
      ).then(() => {
        // Refresh employees list after ensuring all QR codes are updated
        supabase
          .from(TABLES.ATTENDANCE_EMPLOYEES)
          .select('*')
          .order('created_at', { ascending: false })
        ...
        console.error('Error ensuring QR codes:', err)
      })
    ...
    if (employee.qr_code) {
      try {
        ...
        if (parsed.id === employee.id && parsed.employee_code === employee.employee_code) {
          // It's new format and matches current employee
          // IMPORTANT: Only check if employee data changed, NOT if JSON formatting changed
          // This prevents unnecessary regeneration when QR settings load
          const currentData = generateQRCode(employee)
          
          // Compare the actual data, not the string (to avoid false positives from formatting)
          const currentParsed = JSON.parse(currentData)
          const existingParsed = parsed
          
          // Check if any actual employee data changed (not just formatting)
          const dataChanged = 
            currentParsed.id !== existingParsed.id ||
            currentParsed.employee_code !== existingParsed.employee_code ||
            currentParsed.name !== existingParsed.name ||
            currentParsed.job_title !== existingParsed.job_title ||
            currentParsed.department !== existingParsed.department ||
            currentParsed.phone_number !== existingParsed.phone_number ||
            currentParsed.email !== existingParsed.email
          
          if (dataChanged) {
            // Employee data actually changed, update QR code
            console.log('🔄 Employee data changed, updating QR code...')
            return await updateQRCode(employee, currentData)
          }
          
          // Data is the same, return existing QR code (even if formatting differs slightly)
          // This ensures QR code remains stable and scannable
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            <span className="break-words">Employees Management</span>
          </h2>
          ...
          >
            {isImportingFromHR ? (
              ...
              <>
                ...
                <span className="hidden sm:inline">Import from HR Manpower</span>
          ...
          <div className="space-y-4">
            {filteredEmployees.length > 0 ? (
              filteredEmployees.map((employee) => (
                <div key={employee.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-2 p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={async () => {
                        // Ensure QR code exists and is in unified format before showing
                        // This ensures the QR code can be scanned properly by the camera
                        const qrCode = await ensureQRCode(employee)
                        // Verify it's in the correct format (JSON with id and employee_code)
                        try {
                          const parsed = JSON.parse(qrCode)
                          if (!parsed.id || !parsed.employee_code) {
                            // Invalid format, regenerate
                            const newQRCode = generateQRCode(employee)
                            const updatedQRCode = await updateQRCode(employee, newQRCode)
                            setViewingQRCode({ ...employee, qr_code: updatedQRCode })
                          } else {
                            setViewingQRCode({ ...employee, qr_code: qrCode })
                          }
                        } catch (e) {
                          // Not JSON, regenerate in unified format
                          const newQRCode = generateQRCode(employee)
                          const updatedQRCode = await updateQRCode(employee, newQRCode)
                          setViewingQRCode({ ...employee, qr_code: updatedQRCode })
                        }
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setViewingIDCard(employee)}
