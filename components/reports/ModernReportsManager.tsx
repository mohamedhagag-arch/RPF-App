...
type ReportType = 'summary' | 'projects' | 'activities' | 'kpis' | 'financial' | 'performance'
...
export function ModernReportsManager() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (kpisResult.error) {
        console.error('❌ Error fetching KPIs:', kpisResult.error)
  ...
  const getFilteredData = () => {
    ...
    if (selectedProjects.length > 0) {
      ...
      filteredKPIs = kpis.filter(k => selectedProjects.some(pc => 
        k.project_code === pc || k.project_full_code?.startsWith(pc)
      ))
    }
  ...
  const exportToCSV = () => {
    ...
    if (activeReport === 'projects') {
      ...
      filteredKPIs.forEach(k => {
        csv += `"${k.activity_name}","${k.project_full_code}","${k.input_type}","${k.quantity}","${k.unit}","${k.date}","${new Date(k.created_at).toLocaleDateString()}"\n`
      })
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <SmartFilter
              ...
              selectedActivities={[]} // ✅ Added required prop
      ...
      <Card>
        <CardContent className="p-6">
          {activeReport === 'summary' && <SummaryReport summary={summary} />}
...
function SummaryReport({ summary }: { summary: any }) {
  return (
    <div className="space-y-6">
          ...
          <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            {kpis.slice(0, 50).map(kpi => (
              <tr key={kpi.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                  {kpi.project_full_code || kpi.project_code}
                </td>
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                ...
                <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                  {kpi.date ? new Date(kpi.date).toLocaleDateString() : 'N/A'}
                </td>
              </tr>
            ))}
          </tbody>
